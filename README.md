...
- **NLU extraction** — given a chat message and a list of fields to pull out (destination, duration, traveler count, budget tier, vacation pace, transport mode, yes/no), asks the LLM to extract exactly those fields as structured JSON — see ["How NLU Extraction Works"](#how-nlu-extraction-works) below.
...
`POST /nlu/extract` takes a chat message and a list of fields to pull out of it (`destination`, `duration`, `numOfTravelers`, `budget`, `pace`, `transportMode`, or `yesno`), and asks the LLM to return exactly those fields as a JSON object — nothing else, no explanation, `null` for anything genuinely not present in the message.
