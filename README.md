...
- **NLU extraction** — given a chat message and a list of fields to pull out (destination, duration, traveler count, budget tier, vacation pace, transport mode, arrival/departure point, yes/no, or day/spot/category for a targeted itinerary edit), asks the LLM to extract exactly those fields as structured JSON — see ["How NLU Extraction Works"](#how-nlu-extraction-works) below.
...
`POST /nlu/extract` takes a chat message and a list of fields to pull out of it (`destination`, `duration`, `numOfTravelers`, `budget`, `pace`, `transportMode`, `arrivalPoint`, `departurePoint`, `yesno`, `dayNumber`, `targetSpotHint`, or `replacementCategory`), and asks the LLM to return exactly those fields as a JSON object — nothing else, no explanation, `null` for anything genuinely not present in the message.
